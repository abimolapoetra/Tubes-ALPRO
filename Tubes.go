...
func main() {
    ...
    for {
        ...
        fmt.Println("5. Cari Crypto dengan Harga >  (Sequential Search)") // <<-- ganti teks menu
        ...
        case 4:
            deleteCrypto()
        case 5:
            searchByPriceAbove() // <<-- panggil fungsi baru di sini
        case 6:
            searchBinary()
        case 7:
            viewMyCrypto()
...
func loginUser() bool {
	...
	for !loginSukses {
		...
		if index == -1 {
			fmt.Println("Username belum terdaftar, melakukan registrasi...")
			registerUser(uname)
		} else {
...
func registerUser(username string) {
	if userCount >= NMAX {
		fmt.Println("Kuota pengguna penuh, tidak bisa mendaftar.")
		return
	}

	var password string
	fmt.Printf("Masukkan password untuk pengguna \"%s\": ", username)
	fmt.Scan(&password)

	userList[userCount] = User{Username: username, Password: password}
	userCount++
	fmt.Println("Registrasi berhasil! Silakan login kembali.")
}
...
func searchByPriceAbove() {
    fmt.Println("cari Crypto dengan harga ")
    found := false
	var harga float64
	fmt.Print("Masukkan harga:")
	fmt.Scan(&harga)
    for i := 0; i < cryptoCount; i++ {
        if cryptoList[i].Price > harga {
            printCrypto(cryptoList[i])
            found = true
        }
    }
    if !found {
        fmt.Println("Tidak ditemukan crypto dengan harga di atas", harga)
    }
}
